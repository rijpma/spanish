...
b = c(`(Intercept)` = 60, age = -0.3, dum1 = -0.2, dum2 = 0.01, `dum2:year` = 0.2, year = 0.01)
...
for (bin in bins){
    for (i in 1:N){
        ...
        sim_count = data.table::CJ(age = 1:100, year = 0:1, dum1 = 0:1, dum2 = 0:1)
        ...
        sim_count[, N := rnorm(
            n = .N, 
            mean = b["(Intercept)"] + b["age"]*age + b["dum1"]*dum1 + b["dum2"]*dum2 + b["dum2:year"]*year*dum2 + b["year"]*year, 
            sd = sigma)]
        ...
        dat = sim_count[, 
            list(N = sum(N)), 
            by = list(age = floor(age / bin) * bin, dum1, dum2, year)]
        ...
        # knows?
        m = summary(lm(N ~ age + dum1 + dum2 + dum2*year, data = dat))
